...
#'   (filtered to target study). Must contain columns specified by `strStudyMonthCol`
#'   and `strMetricCol`.
...
#'   from `Analyze_StudyKRI_PredictBounds` (optional, filtered to target study).
#'   If provided, must contain: `StudyMonth`, `MedianMetric`, `LowerBound`, `UpperBound`.
#' @param strStudyID character. Study ID to display in title/subtitle.
...
Visualize_StudyKRI <- function(
  ...
  p <- ggplot2::ggplot(dfGroupBounds, ggplot2::aes(x = .data$StudyMonth)) +
    ...
    ggplot2::geom_ribbon(
      ggplot2::aes(ymin = .data$LowerBound, ymax = .data$UpperBound),
      fill = "lightblue",
      alpha = 0.5
    ) +
    # Group median line (dashed blue)
    ggplot2::geom_line(
      ggplot2::aes(y = .data$MedianMetric),
      linetype = "dashed",
      color = "blue",
      linewidth = 0.5
    )
