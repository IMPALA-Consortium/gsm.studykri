...
test_that("mapping workflows execute successfully on resampled data", {
  # Load original data for resampling
  lRaw_original <- list(
    Raw_SITE = clindata::ctms_site,
    Raw_STUDY = clindata::ctms_study,
    Raw_PD = clindata::ctms_protdev,
  ...
  # Resample the study data
  lRaw_resampled <- ResampleStudy(
    lRaw = lRaw_original,
  ...
  # Use suppressWarnings since resampled data may or may not have date issues
  suppressWarnings({
    lIngest <- gsm.mapping::Ingest(lRaw_resampled, gsm.mapping::CombineSpecs(mapping_wf))
  })
  ...
  for (output in expected_outputs) {
    expect_true(
      output %in% names(lMapped),
      info = sprintf("Expected output '%s' not found in lMapped from resampled data", output)
    )
  }
  
  # Verify the resampled data maintained correct studyid
  expect_equal(unique(lMapped$Mapped_SUBJ$studyid), "RESAMPLED001")
  
  # Verify we got approximately the expected number of subjects
  expect_true(nrow(lMapped$Mapped_SUBJ) <= 30)
  expect_true(nrow(lMapped$Mapped_SUBJ) > 0)
})
